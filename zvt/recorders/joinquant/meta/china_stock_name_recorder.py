...
from zvt.domain import StockDetail,StockNames

from zvt.utils.pd_utils import pd_is_not_null
from zvt.utils.time_utils import now_pd_timestamp, to_time_str

class JqChinaStockNamesRecorder(TimeSeriesDataRecorder):
    ...
    def record(self, entity, start, end, size, timestamps):
        ...
        q = query(finance.STK_NAME_HISTORY).filter(
            finance.STK_NAME_HISTORY.code == to_jq_entity_id(entity)).filter(
            finance.STK_NAME_HISTORY.pub_date >= to_time_str(start)).limit(10)
        ...
        if pd_is_not_null(df):
            ...
            df['change_date'] = pd.to_datetime(df['start_date'])
...
__all__ = ['JqChinaStockNamesRecorder']
