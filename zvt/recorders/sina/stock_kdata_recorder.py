...
from zvt.domain import TradingLevel, SecurityType, Provider, StockKdata, StoreCategory
...
class StockKdataSinaSpider(FixedCycleDataRecorder):
    ...
    store_category = StoreCategory.stock_kdata
    data_schema = StockKdata
    ...
    def __init__(self, security_type=SecurityType.stock, exchanges=['sh', 'sz'], codes=None, batch_size=10,
        ...
        for security_item in self.securities:
            kdata = get_kdata(security_id=security_item.id, provider=self.provider,
                              level=self.level, order=StockKdata.timestamp.desc(),
                              return_type='domain',
    ...
    def on_finish(self, security_item):
        ...
        if latest_factor:
            if latest_factor != self.current_factors.get(security_item.id):
                sql = 'UPDATE stock_kdata SET qfq_close=hfq_close/{},qfq_high=hfq_high/{}, qfq_open= hfq_open/{}, qfq_low= hfq_low/{} where ' \
                ...
                sql = 'UPDATE stock_kdata SET qfq_close=hfq_close/{},qfq_high=hfq_high/{}, qfq_open= hfq_open/{}, qfq_low= hfq_low/{} where ' \
                      'security_id=\'{}\' and level=\'{}\''.format(latest_factor, latest_factor, latest_factor,
                                                                   latest_factor, security_item.id, self.level.value)
