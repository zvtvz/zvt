...
class FallBelowTransformer(Transformer):
    ...
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if col not in input_df.columns:
            group_result = group_by_entity_id(input_df['close']).rolling(window=self.window,
        ...
        s = group_by_entity_id(s).rolling(window=self.interval,
                                          min_periods=self.interval).apply(
