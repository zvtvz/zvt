...
def every_day_report():
    while True:
        try:
            ...
            # 计算均线
            my_selector = TargetSelector(start_timestamp='2017-01-01', end_timestamp=now_pd_timestamp())
            # add the factors
            # 设置dry_run为True，因为我们只需要最近的数据，不需要加载全量数据
            ma_factor = CrossMaFactor(start_timestamp='2017-01-01', end_timestamp=now_pd_timestamp(), dry_run=True)
            ...
            msg = '\n'.join(long_targets)
            email_action.send_message("5533061@qq.com", msg, f'{now_pd_timestamp()} 选股结果')
