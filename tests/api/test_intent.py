...
def test_composite():
    composite(entity_id='stock_sz_000338', data_schema=CashFlowStatement,
              columns=[CashFlowStatement.net_op_cash_flows, CashFlowStatement.net_investing_cash_flows,
                       CashFlowStatement.net_financing_cash_flows],
              filters=[CashFlowStatement.report_period == 'year',
                       CashFlowStatement.report_date == to_pd_timestamp('2016-12-31')])
    composite(entity_id='stock_sz_000338', data_schema=BalanceSheet,
              columns=[BalanceSheet.total_current_assets, BalanceSheet.total_non_current_assets,
                       BalanceSheet.total_current_liabilities, BalanceSheet.total_non_current_liabilities],
              filters=[BalanceSheet.report_period == 'year',
                       BalanceSheet.report_date == to_pd_timestamp('2016-12-31')])


def test_composite_all():
    composite_all(entity_ids=None, data_schema=Stock1dKdata,
